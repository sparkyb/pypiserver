...
def auth_arg(arg: str) -> t.Dict[str, t.Optional[t.List[str]]]:
    ...
    for action in arg.split(","):
        ...
        if sep:
            users = list(filter(None, map(str.strip, users.split(":"))))
        else:
            users = None
...
class RunConfig(_ConfigCommon):
    ...
    def __init__(
        ...
        port: int,
        host: str,
        authenticate: t.Union[t.Dict[str, t.Optional[t.List[str]]], t.List[str]],
        ...
        groups: t.Dict[str, t.List[str]] = None,
        **kwargs: t.Any,
    ) -> None:
        """Construct a RuntimeConfig."""
        ...
        self._derived_properties = (
            self._derived_properties + ("auther", "groups")
        )
